...
class SavingProductEntity extends Equatable implements FinancialProduct {
  @override
  final String bankId;
  @override
  ...
  FinancialRate? getRateData(int month) {
    ...
    List<FinancialRate> rateValues = generateKeys(month).map((key) => rates[key]).whereType<FinancialRate>().toList();

    if (rateValues.isEmpty) {
      return null;
    }

    return rateValues.reduce((curr, next) => curr.defaultRate > next.defaultRate ? curr : next);
  }
